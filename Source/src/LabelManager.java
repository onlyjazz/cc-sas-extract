...
class LabelManager
{
    private ArrayList<Label> labels; // Array of Label's managed
    private String oc_label;         // OpenClinica's label for this object
    private int num_labels;          // Number of Label's managed
    ...
    /**
     *  Label addLabel(String options, String values):
     *---------------------------------------------------------------------------
     *  Add the given options and values to the LabelManager. A new label will be
     *  created and added to the ArrayList if this mapping of options to values
     *  is not already represented by this LabelManager.
     *
     *  Takes care of making sure that the sas_label that represents this mapping
     *  is unique and is contained within 32 characters.
     *
     *  Returns the Label that represents this mapping of options to values.
     *
     **/
	...
	for (int i=0; i < labels.size(); i++) {
		...
		if (DEBUG) {
		    System.out.println("LabelManager: Found label in label manager.");
		}
		return label;
	    } else {
		if (DEBUG) {
		    System.out.println("LabelManager: Label not found in label manager.");
		}
	    }
	}
	...
	// Add this mapping to the Label list
	...
	Label label;
	label = lm.addLabel("ST","0---0,1,2","HERE----HERE,Hepinefrin,Dremol");	label.print();
