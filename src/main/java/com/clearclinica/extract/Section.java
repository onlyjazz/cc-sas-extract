...
public class Section extends ContainerItem {
    ...
    public void createGenericColumns() {
        ...
        Column column;
        column = new Column("study", "com.clearclinica.extract.Study Protocol ID", "character", "string", 50, 50, "$CHAR");
        addItem(column);
        column = new Column("study_site", "Site name", "character", "string", 50, 50, "$CHAR");
        addItem(column);
        column = new Column("sid", "com.clearclinica.extract.Study Subject ID", "character", "string", 50, 50, "$CHAR");
        addItem(column);
        column = new Column("study_event", "com.clearclinica.extract.Study Event", "character", "string", 50, 50, "$CHAR");
        addItem(column);
        column = new Column("event_startdt", "Event start date time", "numeric", "datetime", 19, 19, "IS8601DT");
        addItem(column);
        column = new Column("crf_name", "com.clearclinica.extract.Crf Name", "character", "string", 50, 50, "$CHAR");
        addItem(column);
        column = new Column("crf_version", "com.clearclinica.extract.Crf Version", "character", "string", 50, 50, "$CHAR");
        addItem(column);
        //RR
        column = new Column("event_crf_id", "Event com.clearclinica.extract.Crf", "character", "string", 50, 50, "$CHAR");
        addItem(column);
    }
    ...
    public ArrayList<UniqueData> getUniqueData() {
        // Get all com.clearclinica.extract.ColumnData inside this com.clearclinica.extract.Section object
	...
	/*
    for (com.clearclinica.extract.ColumnData cd : data) {
	...
	*/
    ...
    public void writeToXmlFile(BufferedWriter file, String indent) {
        try {
            ...
            System.out.println("IOException in com.clearclinica.extract.Section::writeToXmlFile(): " + e);
        }
    }
    ...
    public void write(BufferedWriter file, String indent, Section.UniqueData d, int ordinal) {
	    ...
	    /* Iterate through all this Sections contained items and
	       have these items write themselves to the xml file */
            ...
            if (hasGroup()) {
                file.write(indent + "  <row_number>" + ordinal + "</row_number>");
                file.newLine();
            }
    ...
    public boolean writeHeader(BufferedWriter file, String indent, Section.UniqueData d) {
        try {
            ...
            if (column_data == null) {
                ...
                // Debug com.clearclinica.extract.Section information:
                this.print_debug();

                ArrayList<Column> columns = this.getAllColumns();

                for (Column column : columns) {
                    System.out.println("com.clearclinica.extract.Column Name: " + column.name());
                    column.printItemsWithData("  ");
                }

                //file.write(indent + "XX<study>"+com.clearclinica.extract.Helper.cleanse(studyProtocol())+"</study>"); file.newLine();
                //file.write(indent + "XX<sid>"+ com.clearclinica.extract.Helper.cleanse(d.sid)+"</sid>"); file.newLine();
                //file.write(indent + "XX<event>"+ com.clearclinica.extract.Helper.cleanse(d.event_id)+"</event>"); file.newLine();

                return false;
            }

            file.write(indent + "<study>" + Helper.cleanse(studyProtocol()) + "</study>");
            file.newLine();
            file.write(indent + "<study_site>" + Helper.cleanse(column_data.site()) + "</study_site>");
            file.newLine();
            file.write(indent + "<sid>" + Helper.cleanse(d.sid) + "</sid>");
            file.newLine();
            file.write(indent + "<study_event>" + Helper.cleanse(d.event_id) + "</study_event>");
            file.newLine();
            //RR
            file.write(indent + "<event_crf_id>" + Helper.cleanse(d.event_crf_id) + "</event_crf_id>");
            file.newLine();
            file.write(indent + "<event_startdt>" + Helper.cleanse(column_data.eventStartDate()) + "</event_startdt>");
            file.newLine();
            file.write(indent + "<crf_name>" + Helper.cleanse(column_data.crfName()) + "</crf_name>");
            file.newLine();
            file.write(indent + "<crf_version>" + Helper.cleanse(column_data.crfVersion()) + "</crf_version>");
            file.newLine();
    ...
    public void writeMapFile(BufferedWriter file) {
        try {
            ...
            file.write("  <TABLE name=\"" + section_name + "\">");
            file.newLine();
            file.write("    <TABLE-DESCRIPTION>" + section_description + "</TABLE-DESCRIPTION>");
            file.newLine();
            file.write("    <TABLE-PATH syntax=\"XPath\">/OCEXPORT/TABLE/" + section_name + "</TABLE-PATH>");
            file.newLine();
            ...
            if (hasGroup()) {
                file.write("    <COLUMN name=\"row_number\">");
                file.newLine();
                file.write("      <PATH syntax=\"XPath\">/OCEXPORT/TABLE/" + section_name + "/row_number</PATH>");
                file.newLine();
                file.write("      <DESCRIPTION>Row number of data in group</DESCRIPTION>");
                file.newLine();
                file.write("      <TYPE>numeric</TYPE>");
                file.newLine();
                file.write("      <DATATYPE>integer</DATATYPE>");
                file.newLine();
                file.write("      <FORMAT>BEST</FORMAT>");
                file.newLine();
                file.write("    </COLUMN>");
                file.newLine();
            }
