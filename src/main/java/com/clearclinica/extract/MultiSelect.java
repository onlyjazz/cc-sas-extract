...
public class MultiSelect extends Column {
    ArrayList<String> columns;     // com.clearclinica.extract.Column names
    ...
    public void writeToMapFile(BufferedWriter file, String section_name) {
	    ...
	    /* Data must be written to the map file for every column that this multi-select item contains */
            ...
            while (itr.hasNext()) {
                ...
                file.write("    <COLUMN name=\"" + Helper.cleanse(column) + "\">");
                file.newLine();
                file.write("      <PATH syntax=\"XPath\">/OCEXPORT/TABLE/" + section_name + "/" + column + "</PATH>");
                file.newLine();
                file.write("      <DESCRIPTION>" + xmlDescription() + " - " + option + "</DESCRIPTION>");
                file.newLine();
                file.write("      <TYPE>numeric</TYPE>");
                file.newLine();
                file.write("      <DATATYPE>integer</DATATYPE>");
                file.newLine();
                file.write("      <FORMAT width=\"2\">BEST</FORMAT>");
                file.newLine();
                file.write("    </COLUMN>");
                file.newLine();
